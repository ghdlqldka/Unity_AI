...
public class WallJumpAgent : Agent
{
    ...
    public UnityEngine.Object noWallBrain;
    // Brain to use when a jumpable wall is present
    public UnityEngine.Object smallWallBrain;
    // Brain to use when a wall requiring a block to jump over is present
    public UnityEngine.Object bigWallBrain;
    ...
    public override void Initialize()
    {
        ...
        if (modelOverrider != null && modelOverrider.HasOverrides)
        {
        ...
        Vector3 targetPos, Rigidbody rb, float targetVel, float maxVel)
    {
        ...
        {
            // Unity 2023.2에서는 velocity 사용 (velocity는 2023.3+)
        ...
        if (jumpingTime > 0f)
        {
            m_JumpTargetPos =
                new Vector3(m_AgentRb.position.x,
                    m_JumpStartingPos.y + (m_WallJumpSettings?.agentJumpHeight ?? 2f),
                    m_AgentRb.position.z) + dirToGo;
            MoveTowards(m_JumpTargetPos, m_AgentRb,
                m_WallJumpSettings?.agentJumpVelocity ?? 10f,
                m_WallJumpSettings?.agentJumpVelocityMaxChange ?? 10f);
        }
    ...
    void ConfigureAgent(int config)
    {
        ...
        if (config == 0)
        {
            ...
            // SetModel(m_NoWallBehaviorName, noWallBrain); // 주석 처리
            Debug.Log($"Configured for no wall with brain: {m_NoWallBehaviorName}");
        }
        else if (config == 1)
        {
            localScale = new Vector3(
                ...
                localScale.z);
            wall.transform.localScale = localScale;
            // SetModel(m_SmallWallBehaviorName, smallWallBrain); // 주석 처리
            Debug.Log($"Configured for small wall with brain: {m_SmallWallBehaviorName}");
        }
        else
        {
            ...
            localScale = new Vector3(
                ...
                localScale.z);
            wall.transform.localScale = localScale;
            // SetModel(m_BigWallBehaviorName, bigWallBrain); // 주석 처리
            Debug.Log($"Configured for big wall with brain: {m_BigWallBehaviorName}");
        }
    }
}